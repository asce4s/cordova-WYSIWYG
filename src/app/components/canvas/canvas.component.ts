...
export class CanvasComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    const containerHeight = $('#elements').innerHeight();
    ...
    this.route.params.subscribe(params => {
      ...
      pages.subscribe((item) => {
        item.forEach((v) => {
          this._pageService.add(v);
          console.log(this._pageService.getAllPages());
        });

        this.db = this.af.database.object('/options/' + this.id);


        this.db.subscribe((opt) => {
          this.loadFirebaseData(opt);
  ...
  private genElement(rElement, nElement, elFunc, clickfunc) {
     ...
     */
    const accessKey = $(rElement).attr('accesskey');
  ...
  public mediaManager(event) {
    if (event) {
      this.mediaModel.show();
    }
  }

  public mediaHandle(event) {
    if (event) {
      this.mediaURL = event.url;
      this.selectedImage.src = event.url;
      this.mediaModel.hide();
      const id = this.selectedImage.id;

      $('#' + id).attr('src', this.mediaURL);
    }
  }
